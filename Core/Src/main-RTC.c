...
void spitstring(struct spit *cl,uint8_t *st)
{
	cl->Mor[0] = st[0]-'0';
	cl->Aft[0] = st[5]-'0';
	cl->Eve[0] = st[10]-'0';
	
	cl->Mor[1] = (st[1]-'0')*10+(st[2]-'0');
	cl->Aft[1] = (st[6]-'0')*10+(st[7]-'0');
	cl->Eve[1] = (st[11]-'0')*10+(st[12]-'0');
	
	cl->Mor[2] = (st[3]-'0')*10+(st[4]-'0');
	cl->Aft[2] = (st[8]-'0')*10+(st[9]-'0');
	cl->Eve[2] = (st[13]-'0')*10+(st[14]-'0');
}

struct spit cassette1;
...
int main(void)
{
  ...
  while (1)
  {
		...
		if(time1.hour==cassette1.Mor[1] &&  time1.minutes==cassette1.Mor[2] && check)
		{
		...
		if(time1.minutes!=cassette1.Mor[2])
		{
				check = 1;
		}
